...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/MULTI-WSS/main/permission/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/MULTI-WSS/main/permission/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/MULTI-WSS/main/permission/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
hargaxray=$(grep -w "Price Xray" /etc/.maAsiss/price | awk '{print $NF}')
    ...
    if [[ "$(grep -w "${message_from_id}" $User_Active | grep -wc 'reseller')" != '0' ]]; then
        ...
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "===========================\n⛔ ACCESS DENIED ⛔\n===========================\n\nfor register to be a reseller contact : $admin_bot_panel\n\n===========================\nBot Panel By : @https://raw.githubusercontent.com/SandakanVPNTrickster/MULTI-WSS/main/permission/ipmini\n===========================\n"
        return 0
    fi
}
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '👤 CREATE USER 👤\n\nUsername:')
                    ...
                    [[ "$sizemax" -gt '10' ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "⛔ Use no maximum 10 characters [EX: https://raw.githubusercontent.com/SandakanVPNTrickster/MULTI-WSS/main/permission/ipmini]")" \
                            --parse_mode html
                        >$CAD_ARQ
                        break
                    }
